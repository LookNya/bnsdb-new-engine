...
class BopaePart extends Component {
	...
	points(dir) {
		...
		let [x1, y1] = [x0+Math.cos(angle-da)*w*dw, y0+Math.sin(angle-da)*w*dw]
		let [x2, y2] = [x0+Math.cos(angle+da)*w*dw, y0+Math.sin(angle+da)*w*dw]
	...
	render() {
		...
		return (
			<svg className="bopae-part" style={{left: w-x0, top: w-y0, width: w, height: w, transform}} onClick={this.onClick}>
				<polygon points={this.points(this.props.dir)} />
				<image height="100%" width="100%" href={process.env.PUBLIC_URL +'/img/'+ this.props.name + this.props.dir +'.png'} />
			</svg>
		)
	}
}
...
class BopaeCircle extends Component {
	onClick = (dir) => {
		this.props.onClick(this.props.name, dir)
	}
	render() {
		return (
			<div className="bopae-circle" style={{width: PART_WIDTH*2, height: PART_WIDTH*2}}>
				{Array(8).fill(0).map((x,i) =>
					<BopaePart key={i} dir={i+1} name={this.props.name} onClick={this.onClick} />
				)}
			</div>
		)
	}
